...
function useCountry(code){
  return useQuery(['country', code], async () => {
    ...
    return response.json()
  }, { keepPreviousData: true, staleTime: twentyFourHours, enabled: !!code, refetchOnWindowFocus: false } )
}

function useCountryNames(borders){
  return useQuery(['codeCountry', borders], async () => {
    const response = await fetch(`https://restcountries.com/v2/alpha?codes=${borders.join(',')}`)
    if(!response.ok){
      throw new Error('Error querying data' + response.status)
    }
    return response.json()
